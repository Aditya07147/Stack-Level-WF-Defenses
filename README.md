...
|-------------------------|:--------:|-------------------------|
| **Model 1: Clean Baseline** | **90.00%** | In an undefended standard Linux stack, metadata (burst sizes, raw pacing, volumes) highly leaks site identities onto the wire . |
| **Cross-Evaluation (Clean ➔ Stob)** | **10.00%** | Applying Model 1 on Stob-defended traces renders the model useless, dropping classification performance to random chance . |
| **Model 2: Adaptive Attacker** | **30.00%** | Training a new classifier directly on defended traffic recovers some signature, but the defense still successfully degrades identification by 60% . |
